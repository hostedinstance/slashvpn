...
void main(){
  ...
  fragColor     = vec4(color * intensity, intensity);
    ...
    const stops = ['#3B0FCC', '#000', '#2200aa'].map(hex => {
    ...
    const animate = (t: number) => {
      ...
      program.uniforms.uTime.value = t * 0.001 * 1.8;
...
export function AuroraFooter() {
  ...
  return (
    <div className="relative overflow-hidden" style={{ backgroundColor: '#000', minHeight: '320px' }}>
      ...
      {/* Aurora — renders on black via screen blend */}
      <AuroraCanvas />
        ...
        <div className="mx-auto max-w-5xl px-6 flex flex-col items-center gap-8">
          ...
          <Link href="/" aria-label="Главная">
            <Image
              ...
              width={220}
              height={40}
              className="h-10 w-auto opacity-50 hover:opacity-85 transition-opacity duration-300"
            />
          </Link>

          {/* Social icons — bigger on desktop, responsive */}
          <div className="flex items-center gap-6 md:gap-8">
            {SOCIALS.map(s => (
              <Link
                ...
                aria-label={s.label}
                className="
                  block text-white/30 hover:text-white/80
          ...
          <div className="flex flex-wrap justify-center gap-5 md:gap-8">
            {[
              ...
              <Link
                ...
                style={{ color: 'rgba(180,205,255,0.28)' }}
                onMouseEnter={e => ((e.target as HTMLElement).style.color = 'rgba(200,215,255,0.70)')}
                onMouseLeave={e => ((e.target as HTMLElement).style.color = 'rgba(180,205,255,0.28)')}
              >
                {label}
              </Link>
            ))}
          ...
          {/* Copyright */}
          <p className="font-inter-tight text-xs" style={{ color: 'rgba(140,175,255,0.20)' }}>
            © {year} SLASH SOLUTIONS
          </p>
