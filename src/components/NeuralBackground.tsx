...
export interface NeuralBackgroundProps {
  /** Дополнительные стили на корневой div */
 ...
 * NeuralBackground
 * ─────────────────────────────────────────────────────────────────────────────
 * Рендерит CPPN-шейдер через Three.js на весь переданный контейнер.
 *
 * Используй внутри позиционированного родителя (fixed/absolute inset-0)
 * для корректного заполнения.
 *
 * Компонент сам занимает position:absolute inset:0 — не добавляй
 * ширину/высоту снаружи, позаботься только о позиционировании родителя.
 */
export function NeuralBackground({ style }: NeuralBackgroundProps) {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(
      wrapRef.current,
      { filter: 'blur(20px)', scale: 1.04, autoAlpha: 0 },
      { filter: 'blur(0px)', scale: 1, autoAlpha: 1, duration: 1.4, ease: 'power3.out', delay: 0.1 }
    );
  }, { scope: wrapRef });
